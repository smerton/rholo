...
void silo(VD*x,VD*d,VD*p,VD*e,VD*q,VD*c,VD*u,VI*m,int step,double time){
  ...
  int nx((x->size()-1));               // number of cells in x direction
  ...
  int lnodelist=(4*nzones);            // length of the node list
...
// output arrays
  ...
  double var1[nzones];                         // zone centred scalars
  double var2[nnodesk];                        // node centred scalars
...
// connectivities
  ...
  for(int i=0;i<nx;i++){
    for(int j=0;j<2;j++){
      for(int k=0;k<2;k++){
        if(j==0){
          nodelist[4*i+(j*2)+k]=i+k;
        }else{
          nodelist[4*i+(j*2)+k]=(i+nx+1)-k+1;
        }
      }
    }
  }
...
// zone shapes
  ...
  for(int i=0;i<nshapetypes;i++){shapesize[i]=2*2;}
...
// write scalar data fields
  ...
  for(long i=0;i<nzones;i++){var1[i]=d->at(i);}
  optlist = DBMakeOptlist(1);
  dberr=DBAddOption(optlist, DBOPT_UNITS, (void*)"g/cc");
  dberr=DBPutUcdvar1(dbfile,"density","Elements",var1,nzones,NULL,0,DB_DOUBLE,DB_ZONECENT,optlist);
  dberr=DBFreeOptlist(optlist);

  for(long i=0;i<nzones;i++){var1[i]=p->at(i);}
  optlist = DBMakeOptlist(1);
  dberr=DBAddOption(optlist, DBOPT_UNITS, (void*)"Mb");
  dberr=DBPutUcdvar1(dbfile,"pressure","Elements",var1,nzones,NULL,0,DB_DOUBLE,DB_ZONECENT,optlist);
  dberr=DBFreeOptlist(optlist);

  for(long i=0;i<nzones;i++){var1[i]=q->at(i);}
  optlist = DBMakeOptlist(1);
  dberr=DBAddOption(optlist, DBOPT_UNITS, (void*)"Mb");
  dberr=DBPutUcdvar1(dbfile,"bulk_q","Elements",var1,nzones,NULL,0,DB_DOUBLE,DB_ZONECENT,optlist);
  dberr=DBFreeOptlist(optlist);

  for(long i=0;i<nzones;i++){var1[i]=c->at(i);}
  optlist = DBMakeOptlist(1);
  dberr=DBAddOption(optlist, DBOPT_UNITS, (void*)"cm/s");
  dberr=DBPutUcdvar1(dbfile,"sound_speed","Elements",var1,nzones,NULL,0,DB_DOUBLE,DB_ZONECENT,optlist);
  dberr=DBFreeOptlist(optlist);

  for(long i=0;i<nzones;i++){var1[i]=e->at(i);}
  optlist = DBMakeOptlist(1);
  dberr=DBAddOption(optlist, DBOPT_UNITS, (void*)"Mbcc");
  dberr=DBPutUcdvar1(dbfile,"energy","Elements",var1,nzones,NULL,0,DB_DOUBLE,DB_ZONECENT,optlist);
  dberr=DBFreeOptlist(optlist);

// write vector data fields

  for(long i=0;i<x->size();i++){var2[i]=u->at(i);};for(int i=0;i<x->size();i++){var2[x->size()+i]=u->at(i);}
  optlist = DBMakeOptlist(1);
  dberr=DBAddOption(optlist, DBOPT_UNITS, (void*)"cm/s");
  dberr=DBPutUcdvar1(dbfile,"velocity_x","Elements",var2,nnodesk,NULL,0,DB_DOUBLE,DB_NODECENT,optlist); 
  dberr=DBFreeOptlist(optlist);

  for(long i=0;i<x->size();i++){var2[i]=0.0;};for(int i=0;i<x->size();i++){var2[x->size()+i]=0.0;}
  optlist = DBMakeOptlist(1);
  dberr=DBAddOption(optlist, DBOPT_UNITS, (void*)"cm/s");
  dberr=DBPutUcdvar1(dbfile,"velocity_y","Elements",var2,nnodesk,NULL,0,DB_DOUBLE,DB_NODECENT,optlist); 
  dberr=DBFreeOptlist(optlist);
