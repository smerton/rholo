      ...
      for(int iloc=0;iloc<S3.nloc();iloc++){
        ...
        for(int jloc=0;jloc<S3.nloc();jloc++){
          ...
          for(int gi=0;gi<S3.ngi();gi++){
            ...
            nnx+=S3.value(iloc,gi)*S3.dvalue(jloc,gi)*S3.wgt(gi);      // divergence term if applying jump condition
            nxn+=S3.dvalue(iloc,gi)*S3.value(jloc,gi)*S3.wgt(gi);      // divergence term if applying Green's theorem
          }
          A.write(iloc,jloc,nn);                // commit to address space in the matrix class
          b[iloc]-=(nnx*u1[S3.nloc()*i+jloc]*p[i])/d[i]; // source - if using jump
//          b[iloc]-=p[i]*(nnx*u1[S3.nloc()*i+jloc]+(S3S[iloc][jloc]*(pstar[jloc]*ustar[jloc]-p[i]*ustar[jloc])))/d[i]; // source - if using jump
//          b[iloc]+=(nxn*u0vol[jloc]-S3S[iloc][jloc]*ustar[jloc])*p[i]/d[i]; // source - if applying Green's theorem
        }
      }
...
// assemble acceleration field for one element
      ...
      for(int iloc=0;iloc<S3.nloc();iloc++){
        ...
        for(int jloc=0;jloc<S3.nloc();jloc++){
          ...
          for(int gi=0;gi<S3.ngi();gi++){
            nn+=S3.value(iloc,gi)*S3.value(jloc,gi)*S3.wgt(gi)*dx/2.0; // mass matrix
            nnx+=S3.value(iloc,gi)*S3.dvalue(jloc,gi)*S3.wgt(gi);      // grad term if applying jump condition
            nxn+=S3.dvalue(iloc,gi)*S3.value(jloc,gi)*S3.wgt(gi);      // grad term if applying Green's theorem
          }
          A.write(iloc,jloc,nn);                // commit to address space in the matrix class
          b[iloc]-=(nnx*p[i]+(S3S[iloc][jloc]*(pstar[jloc]-p[i])))/d[i]; // source - if using jump
//          b[iloc]+=(nxn*pvol[jloc]-S3S[iloc][jloc]*pstar[jloc])/d[i]; // source - if applying Green's theorem
        }
      }
