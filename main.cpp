...
//    for(int i=0;i<NSAMPLES;i++){cout<<r0x[i]<<" "<<R0.density(i)<<" "<<R0.pressure(i)<<" "<<R0.velocity(i)<<" "<<R0.energy(i)<<endl;} // exact solution from Riemann solver
    for(int i=2;i<n+2;i++){cout<<0.5*(x1[i]+x1[i+1])<<" "<<d[i]<<" "<<p[i]<<" "<<e1[i]<<" "<<0.5*(u1[i]+u1[i+1])<<" "<<c[i]<<" "<<q[i]<<endl;} // finite difference solution
...
// advance the solution for the new time step
    ...
    for(int i=0;i<ng;i++){
      double l(x0[i+1]-x0[i]),div(V1[i]-V0[i]),dx(l),du((u0[i+1]-u0[i])/dx);q.at(i)=0.0;
      if(du!=0.0&&div<0.0){q.at(i)=(cq*d[i]*pow(l*du,2))-(cl*d[i]*c[i]*l*pow(du,2)/du);}
    }
