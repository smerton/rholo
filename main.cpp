...
// fluid frame (the Lagrangian frame) using a finite difference method (cell centred
...
// git push -vu NAS finite-difference
//
...
void silo(VVD*x,VD*d,VD*p,VD*e,VD*q,VD*c,VVD*u,VI*mat,int step,double time); // silo graphics output
...
// global data
  ...
  int const n(M.NCells()),ng(M.NSides());               // no. ncells, no. ghosts
  ...
  vector<vector<double> > u0(M.NDims()),u1(M.NDims());  // node velocity
  vector<vector<double> > x0(M.NDims()),x1(M.NDims());  // node coordinates
  ...
  double l[3]={1.0,0.0,1.0},r[3]={0.125,0.0,0.1};       // left/right flux states for Sod problem
//  double l[3]={1.0,-2.0,0.4},r[3]={1.0,2.0,0.4};      // left/right flux states for 123 problem
//  double l[3]={1.0,0.0,1000.0},r[3]={1.0,0.0,0.01};   // left/right flux states for blast wave problem

  vector<vector<double> > mat_data{{1.0,  0.0,0.0,1.0}, // initial flux states d,u,v,p in material 1
                                   {0.125,0.0,0,0,0.1}};// initial flux states d,u,v,p in material 2

// initialise the problem

  M.CopyCoords(x0);
  M.CopyCoords(x1);
  for(int i=0;i<n;i++){p.at(i)=mat_data[M.Material(i)-1][3];}
  for(int i=0;i<n;i++){d0.at(i)=mat_data[M.Material(i)-1][0];}
  for(int i=0;i<n;i++){d1.at(i)=mat_data[M.Material(i)-1][0];}
  for(int i=0;i<n;i++){e0.at(i)=E(d0[i],p[i]);}
  ...
  for(int i=0;i<n;i++){m.at(i)=d0[i]*V0[i];}
  for(int i=0;i<n;i++){ke+=0.0;}
  for(int i=0;i<n;i++){ie+=e1[i]*m[i];}

  for(int idim=0;idim<M.NDims();idim++){
    u0.at(idim).resize(M.NNodes());
    u1.at(idim).resize(M.NNodes());
    for(int i=0;i<M.NCells();i++){
      for(int j=0;j<M.NVertices(i);j++){
        u0.at(idim).at(M.Vertex(i,j))=mat_data[M.Material(i)-1][1+idim];
        u1.at(idim).at(M.Vertex(i,j))=mat_data[M.Material(i)-1][1+idim];
      }
    }
  }
