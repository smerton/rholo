...
#define NSAMPLES 2000    // number of sample points for the exact solution
...
// global data
  ...
  int const n(20),ng(n+2),order(1);                     // no. ncells, ghosts and element order
...
// evolve the Riemann problem to the current time level
    ...
    for(long i=0;i<NSAMPLES;i++){rx.push_back(i/NSAMPLES);}
    R.profile(&rx,time);
...
// fluxes on left and right sides of face 0 (left boundary of cell)
      ...
      l[0]=d[i-1];l[1]=u0[S2.nloc()*(i+1)-1];l[2]=p[i-1];
...
// impose a constraint on the acceleration field at domain boundaries to stop the mesh taking off
    ...
    for(int i=0;i<S2.nloc();i++){u1.at(i)=u0[i];u1.at(S2.nloc()*ng+i)=u0[S2.nloc()*ng+i];}
